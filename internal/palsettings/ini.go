...
func ParseINI(content string) (map[string]string, error) {
	...
	return splitPairs(normalizeParenValues(body)), nil
}

// normalizeParenValues 把未加引号的括号列表值（实机形态
// CrossplayPlatforms=(Steam,Xbox,PS5,Mac)）规范化为带引号形式，
// 使后续按引号切分能整段识别为单值而非被逗号截断。
// 引号内的 =( 不处理（字符串值可能含该形态）；实机列表值无嵌套括号，
// 取最近的 ')' 收尾。
func normalizeParenValues(body string) string {
	...
	for i := 0; i < len(runes); i++ {
		...
		if r == '=' && !inQuote && i+1 < len(runes) && runes[i+1] == '(' {
			j := i + 2
			for j < len(runes) && runes[j] != ')' {
				j++
			}
			if j < len(runes) {
				sb.WriteString(`="` + string(runes[i+1:j+1]) + `"`)
				i = j
				continue
			}
		}
...
// generatedHeader 裸 OptionSettings 行落盘前补全的完整 ini 头
//（生成注释 + PalGameWorldSettings 段头）。PalServer 只应用带段头的配置，
// 缺失段头还会导致下次 LoadGameINI 报「未找到段」。
const generatedHeader = "; Generated by PalPanel\n" + sectionHeader + "\n"

// SaveGameINI 将 content 写入游戏 ini 路径（自动创建目录）。
// content 为裸 OptionSettings 行（Serialize 输出）时自动补全为完整合法 ini：
// 首行注释 + 段头 + OptionSettings 行；已含段头的完整 ini 内容原样写入。
